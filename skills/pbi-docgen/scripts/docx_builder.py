...
def _add_paragraph_border(paragraph, color_hex: str = "D0D0D0", left_only: bool = False) -> None:
    """Add a border to a paragraph (for code block styling).

    Creates pBdr XML with either a left-only accent border (DigitalOcean style)
    or a four-sided thin border.

    Args:
        paragraph: A python-docx Paragraph object.
        color_hex: Border color hex string without '#'.
        left_only: If True, only add a thick left border (accent style).
    """
    color_clean = color_hex.lstrip('#')
    if left_only:
        borders_xml = (
...
def _inline_children_to_text(children: list) -> str:
    """Extract plain text from inline token children (used for heading text)."""
    parts = []
...
def _render_inline_children_to_runs(para, children: list, code_style=None) -> None:
    """Render inline token children as formatted Word runs on a paragraph."""
    bold = False
    italic = False
    for child in children:
...
def _collect_table_rows_from_tokens(tokens: list, start: int):
    """Collect table row data from the token stream starting at a table_open token.

    Returns (rows, consumed) where rows is a list of lists of strings
    and consumed is the number of tokens to advance.
    """
    rows = []
    current_row = []
